...
export default function scriptProcessor() {
    ...
    const processedFiles: Record<string, ParseResult> = {};

    function writeFile(filePath: string, content: string) {
        fs.writeFileSync(filePath, content, {flag: 'a'});
    }
    
    function writeReturn2(filePath: string) {
    ...
    function processFile(filePath: string) {
        ...
        if (res.script) {
            ...
            const script = processScript(res.script, parentDirPath, isMixin);
            if (script == null) {
                return;
            }
            ...
            processedFiles[filePath] = script[1];
        }
    }

    function processScript(block: SFCBlock, parentDirPath: string, isMixin: boolean): [string, ParseResult] | undefined {
        let result = "";
        ...
        function propertyDefinitionStmt(stmt: TSESTree.PropertyDefinition): string | undefined {
            ...
            const name = expr(stmt.key as TSESTree.Expression);

            // record method name
            if (stmt.accessibility !== 'private') {
        ...
        function methodDefinitionStmt(methodDefinitionStmt: TSESTree.MethodDefinition): string | undefined {
            ...
            // record method name
            if (methodDefinitionStmt.accessibility !== 'private') {
                methods.push(name);
            }
        ...
        return [document, { methods, properties }];
    }
